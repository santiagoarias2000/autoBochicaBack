...
exports.SQL_PERSON_FILTER = {
    FILTERCERTIFICATE: "select id_person,first_name, second_name, first_last_name, second_last_name, mail, birthdat, direction, phone_number, occupation, numbers, issued_in, tsdo.type_document, stsscer.state_certificate, value_receipt, photo, photo_fingerprint, date_tuition,vhi.license_plate,tuis.name_tuition,cou.name_course, cou.time_course,tg.type_gender,ts.type, ts.rh, stra.name_stractum, sis.name_sisben, cs.type_civil_status,el.name_education_level, ci.name_city \
    from persons ps\
	...
	where stsscer.state_certificate = 'Certificado'",
    FILTERNOCERTIFICATE: "select id_person,first_name, second_name, first_last_name, second_last_name, mail, birthdat, direction, phone_number, occupation, numbers, issued_in, tsdo.type_document, stsscer.state_certificate, value_receipt, photo, photo_fingerprint, date_tuition,vhi.license_plate,tuis.name_tuition,cou.name_course, cou.time_course,tg.type_gender,ts.type, ts.rh, stra.name_stractum, sis.name_sisben, cs.type_civil_status,el.name_education_level, ci.name_city \
    from persons ps \
    INNER join types_documents tsdo ON tsdo.id_type_document = ps.id_type_document\
    inner join vehicles vhi ON vhi.id_vehicle = ps.id_vehicle\
    inner join tuitions tuis ON tuis.id_tuition = ps.id_tuition\
    ...
    inner join citys ci ON ci.id_city = ps.id_city\
    inner join state_certificate stsscer ON stsscer.id_state_certificate = ps.id_state_certificate\
	where stsscer.state_certificate = 'No Certificado'"
};
