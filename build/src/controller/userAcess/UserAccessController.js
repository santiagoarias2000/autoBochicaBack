"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const sql_access_login_1 = require("./../../repo/usersAccess/sql_access_login");
const UserAccessLogin_1 = __importDefault(require("../../dao/userAccess/UserAccessLogin"));
class UserAccessController extends UserAccessLogin_1.default {
    searchOneAccess(req, res) {
        const mail_access = req.body.mail;
        const password_access = req.body.password;
        const parametro = [mail_access, password_access];
        UserAccessController.searchIdAccess(sql_access_login_1.SQL_ACCESS_LOGIN.VALIDATELOGIN, parametro, res);
    }
}
const userAccessController = new UserAccessController();
exports.default = userAccessController;
